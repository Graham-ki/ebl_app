...
END;
$$;
--on auth insert,create new user
create 
...
CREATE OR REPLACE FUNCTION "public"."handle_new_user"() RETURNS "trigger"
    ...
    AS $$
BEGIN
  -- Check if 'avatar_url' is null or empty in raw_user_meta_data
  IF NEW.raw_user_meta_data->>'avatar_url' IS NULL OR NEW.raw_user_meta_data->>'avatar_url' = '' THEN
    -- Set a default avatar URL if it's missing
    NEW.raw_user_meta_data = jsonb_set(
      NEW.raw_user_meta_data,
      '{avatar_url}',
      '"https://pixabay.com/vectors/avatar-icon-placeholder-facebook-1577909/"'::jsonb
    );
  END IF;
  ...
  -- Return the NEW record to allow the trigger to proceed
  RETURN NEW;
END;
$$;
...
CREATE POLICY "Allow all operations for authenticated users" ON "public"."order_item" USING (true) WITH CHECK (true);



CREATE POLICY "Enable delete for admins" ON "public"."product" FOR DELETE TO "authenticated" USING ((( SELECT "users"."type"
   FROM "public"."users"
